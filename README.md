...
| 9 个广告流程节点可视化与执行 | ✅ |
| **Chat 节点右侧聊天面板**（LangChain，点击节点即聊） | ✅ |
| **产出解析**（LLM JSON → brief/story/styleBible/assetPlan/storyboard） | ✅ |
| **会话联动执行**（聊天产物实时成为节点输出，下游自动消费） | ✅ |
| **Mock/真实双模式**（无 key 也可端到端跑通，填 key 自动切换） | ✅ |
| Memory 节点定义与占位 runtime | ✅（完整版见 P1.5） |
| **Generator 图片生成闭环**（提示词包→生成→资产登记） | ✅ |
| **Asset 版本化**（Artifact Revision 不可变快照，本地文件） | ✅ |
| **图片网格 + 灯箱**（执行结果图片缩略图展示，ImageGrid 雏形） | ✅ |
| **Mock/真实图片双模式**（离线 SVG 占位图，配 key 切真实） | ✅ |
| 资产 API（列表/详情/版本/上传）+ 静态图片服务 | ✅ |
| Asset 节点（资产库原语） | ✅ |
...
3. **点任意 Chat 节点（如「创意策略」）→ 右侧弹出聊天面板**，发消息对话；
   模型回复中的 JSON 产物（简报/剧本/分镜表…）自动解析到节点输出；
4. 点「执行画布」→ 底部弹出逐节点执行结果（下游自动消费聊天产物）。

> **Mock / 真实模式**：未配置 `OPENAI_API_KEY` 时，聊天后端用模板占位
> （标注 Mock），整条链路仍可跑通；在 `.env` 填入 key 后自动切换 LangChain 真实模型。
